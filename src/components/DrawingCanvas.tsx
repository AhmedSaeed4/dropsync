...
export function DrawingCanvas({ onSave, onCancel, onDraw, theme, bgColor }: DrawingCanvasProps) {
  ...
  // Update background color in real-time when bgColor changes
  useEffect(() => {
    if (apiRef.current) {
  ...
  if (isFullscreen) {
    return (
      <div className="fixed inset-0 z-[999] bg-black/40 flex items-center justify-center p-4">
        <div className={`w-full h-full max-w-[1200px] ${isDark ? 'bg-[#0D0D0D]' : 'bg-[#FAF7F2]'} ${roundedClass} flex flex-col overflow-hidden shadow-2xl`}>
          <div className="flex-1 min-h-0 flex flex-col p-3 gap-3">
            {canvasContent}
          </div>
          <div className={`shrink-0 px-4 py-3 border-t ${isDark ? 'border-white/10' : 'border-[#1a1a1a]/10'}`}>
            {buttonBar}
          </div>
        </div>
      </div>
